...
extern Vec<Image> block;
extern Vec<int> righttop;
extern Vec< Vec<int> > rightmap;

extern Vec< Vec<int> > leftmap;
extern Vec<int> lefttop;

extern Vec< Vec<int> > rustmap;
...
{
	static VecMap<int64, Image> tiled;
	static VecMap<Tuple<int64, Point, dword>, int> blocki;
...
void BeautifyMap()
{
	...
	Beautify(RasterImg::dirt(), 3);
	Beautify(RasterImg::dirt(), 4);
	Beautify(RasterImg::dirt(), 5, TUNNEL);
	...
	Beautify(RasterImg::Stones(), 1);
	Beautify(RasterImg::Stones(), 2);

	Beautify(RasterImg::granit(), 10);
	Beautify(RasterImg::granit(), 11);
	...
	for(int x = 0; x <= rmsz.cx; x++) {
		righttop.At(x) = GetBlock(RasterImg::granit(), x, 0, LINE_TOP);
		for(int y = 0; y <= rmsz.cy; y++) {
			rightmap.At(y).At(x) = GetBlock(RasterImg::granit(), x, y, 0);
		}
	}
	...
	for(int x = 0; x <= ltsz.cx; x++) {
		lefttop.At(x) = GetBlock(RasterImg::dirt(), x, 0, LINE_TOP);
		for(int y = 0; y <= ltsz.cy; y++) {
			leftmap.At(y).At(x) = GetBlock(RasterImg::dirt(), x, y, 0);
		}
	}
