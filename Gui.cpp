...
#define TOPICFILE <JetStory/app.tpp/all.i>
#include <drx/Core/topic_group.h>

Image TitleScreen()
{
	static Image graphics = StreamRaster::LoadTxtAny(Txt(screen, screen_length));
	return graphics;
}
...
struct IngameGuiCtrl : TopWindow {
	...
	ActiveText& Add(tukk text, Event<> action)
	{
...
struct RedefineKeys : IngameGuiCtrl {
	...
	virtual void Paint(Draw& w) {
		...
		for(int i = 0; i < 6; i++) {
			...
			Txt k = SDL_GetScancodeName((SDL_Scancode)game_key[i]);
			if(keyi != i || (n & 1)) {
	...
	virtual bool Key(dword key, int count) {
		...
		for(int i = 0; i < SDL_NUM_SCANCODES; i++)
			if(keystate[i]) {
				tukk s = SDL_GetScancodeName((SDL_Scancode)i);
				if(s && *s) {
					game_key[keyi] = i;
					if(++keyi >= 6)
						Break();
...
Txt IngameGui(bool esc)
{
	...
	Array<Sav> sf;
	FindFile ff(AppendFName(ConfigFolder(), "*.sav"));
	while(ff) {
		if(ff.IsFile()) {
			Vec<Txt> h = Split(ff.GetName(), '_');
			if(h.GetCount() == 3) {
				Sav& s = sf.Add();
...
/*		h.Add("5. Info", [&] {
		...
		auto vol = [=]()->Txt { return "5. Громкость звука " + AsTxt(volume) + " / 10"; };
		Button *b = &h.Add(vol(), [&] {
			volume = volume + 1;
			if(volume > 10)
				volume = 0;
