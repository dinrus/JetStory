...
#include <drx/VirtualGui/SDL2GL/SDL2GL.h>
...
#define IMAGECLASS RasterImg
#define IMAGEFILE <JetStory/Raster.iml>
#include <drx/Draw/iml_header.h>
...
extern Vec<Rect> training_block;

inline int TrainingIndex() { return training_total - training_remains; }

void SetMessage(const Txt& msg, bool fl = false);

void PaintMessage();
void TrainingPoint();

extern Txt checkpoint_save;

inline
int GameTick() { return game_tick; }

extern Vec<Image> lflame;
extern Vec<Image> sflame;
extern Vec<Image> bflame;
...
extern Vec<SDL_GameController *> GamePad;
extern Vec<SDL_Joystick *> Joystick;
extern Vec<bool> JoystickButton;
...
Txt StoreGame();
bool   LoadGame(const Txt& h);
Pointf LoadShipPosition(const Txt& h);
...
enum { MAPX = 128, MAPY = 512, BLOCKSIZE = 64 };
...
void PlaySound(int priority, double pan, tukk s, double volume = Null, double freq = Null);
...
// permanent flags, not serialized:
	...
	void Beep(int priority, tukk s, double volume = Null, double freq = Null, double max_distance = 1000);
	void Beep(tukk s, double volume = Null, double freq = Null) { Beep(0, s, volume, freq); }
...
void RegisterEnemy(tukk id, tukk type_id, void (*factory)(One<Enemy>&), bool design = true);
