...
void PaintScene(Size sz, Draw& w, Event<int, int, const Image&> di)
{
	...
	static Vec<int> stars;
	static Vec<Color> star_color;
	...
	if(gameover && tick - gameover_time > 100) {
		...
		
		{
			...
			Txt text = Txt() << "Вы уничтожили " << base_count_max - base_count
			              << " / " << base_count_max << " баз";
			Size tsz = GetTextSize(text, fnt);
			Point p = Rect(window_size).CenterPos(tsz);
			for(int x = -1; x <= 1; x++)
		...
		if(ship.fuel <= 0) {
			...
			Txt text = "У вас закончилось топливо";
			Size tsz = GetTextSize(text, fnt);
			Point p = Rect(window_size).CenterPos(tsz);
			for(int x = -1; x <= 1; x++)
				for(int y = -1; y <= 1; y++)
		...
		if(tick - gameover_time > 350) {
			Font fnt = Arial(50).Bold().Italic();
			Txt text = "Нажмите любую клавишу для старта с последней КТ!";
			Size tsz = GetTextSize(text, fnt);
			Point p = Rect(window_size).CenterPos(tsz);
			for(int x = -1; x <= 1; x++)
				for(int y = -1; y <= 1; y++)
	...
	if(checkpoint_info && tick - checkpoint_info < 1000) {
		Font fnt = Arial(50).Bold().Italic();
		Txt text = "ДОСТИГНУТА КОНТРОЛЬНАЯ ТОЧКА";
		Size tsz = GetTextSize(text, fnt);
		Point p = Rect(window_size).CenterPos(tsz);
	...
	auto paint_val = [&](int x0, int val, int total, Color c, int w1, int w2, tukk text, bool ar = true) {
		x0 += margin;
	...
	auto paint_n = [&](const Image& m, int n, Color color = LtRed()) {
		...
		w.DrawText(x0 + isz.cx + vmargin, nty, AsTxt(n), nfont,
	...
	Txt text = Format("%2d:%02d%s", (int)tm.hour % 12, (int)tm.minute, tm.hour >= 12 ? "pm" : "am");
